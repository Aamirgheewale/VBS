...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    const itemsRes = await pool.query(`
      SELECT ci.id, ci.quantity, b.id AS book_id, b.title, b.author, b.price, b.cover_image_url AS image
      FROM cart_items ci
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (!user_id || !book_id || !quantity || quantity < 1) {
    return new Response(JSON.stringify({ error: 'user_id, book_id, quantity (>=1) required' }), { status: 400 });
  }
  try {
    let cartId;
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  if (!id || quantity == null || isNaN(quantity) || quantity < 1) {
    return new Response(JSON.stringify({ error: 'cart_item id and valid quantity (>=1) required' }), { status: 400 });
  }
  try {
