...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    // Adjusted: look up by id, not email
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!updateData.id) return new Response(JSON.stringify({ error: 'No id' }), { status: 400 });
    ...
    if (fields.length === 0) return new Response(JSON.stringify({ error: 'Nothing to update' }), { status: 400 });

    values.push(updateData.id);
    const sql = `UPDATE users SET ${fields.join(', ')} WHERE id = $${idx} RETURNING id, name, email, phone`;
    const result = await pool.query(sql, values);
